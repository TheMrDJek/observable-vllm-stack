...
Приложения в обоих режимах всегда отправляют OTLP на `alloy:4317`. Меняются только sinks самого Alloy — поэтому переключение `local` → `remote` не требует трогать LiteLLM, Open WebUI или перезапускать модель.
...
|---|---|---|
| `observability local` | Postgres, LiteLLM, Open WebUI, Alloy **+** Prometheus, Loki, Tempo, Grafana | Стенд без внешнего мониторинга |
