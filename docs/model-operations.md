...
| Модель на другом сервере | [Рецепт 4](#рецепт-4-vllm-на-отдельном-сервере) |
| Несколько серверов на одну модель | [Рецепт 5](#рецепт-5-пул-серверов-под-одним-именем-модели) |
...
## Рецепт 4: vLLM на отдельном сервере

До этого места все рецепты про слоты — процессы vLLM на **этой** машине, которыми управляют профили Compose. Модель может жить и на другом сервере: LiteLLM не знает, где она, он знает только `api_base`.

Это способ расширять пул, когда GPU на текущем хосте закончились.

**1. На удалённом хосте поднимите только vLLM.** Остальной стек там не нужен — ни LiteLLM, ни PostgreSQL, ни шлюз:

```bash
docker run -d --name vllm-remote --gpus all --ipc host --restart unless-stopped \
  -p 8000:8000 \
  -v ~/.cache/huggingface:/root/.cache/huggingface \
  -e VLLM_API_KEY="$REMOTE_VLLM_KEY" \
  vllm/vllm-openai:v0.27.1 \
  QuantTrio/Qwen3.5-9B-AWQ --revision 938f8e3ef86c9d1e9bec3705e149694c172592f1 \
  --served-model-name qwen3.5-9b-awq --max-model-len 8192 \
  --gpu-memory-utilization 0.85 --reasoning-parser qwen3
```

Ключ у каждого сервера **свой**. Не переиспользуйте `VLLM_API_KEY` локального стека: скомпрометированный ключ одного хоста не должен открывать остальные.

**2. Добавьте ключ удалённого хоста в `.env`:**

```dotenv
REMOTE_VLLM_KEY=sk-...
```

и пробросьте его в LiteLLM через `compose.yaml`, рядом с существующим `VLLM_API_KEY`:

```yaml
      REMOTE_VLLM_KEY: ${REMOTE_VLLM_KEY:-}
```

**3. Добавьте запись в [config/litellm.yaml](../config/litellm.yaml):**

```yaml
  - model_name: qwen3.5-9b-awq
    litellm_params:
      model: openai/qwen3.5-9b-awq
      api_base: http://10.0.0.5:8000/v1
      api_key: os.environ/REMOTE_VLLM_KEY
      timeout: 600
```

**4. Примените, не трогая модели:**
...
Локальные слоты при этом не перезапускаются — пересоздаётся только LiteLLM, простой измеряется секундами.

### Что удалённая модель меняет в эксплуатации

| Что | Локальный слот | Удалённый сервер |
|---|---|---|
| `model.sh start/stop/status` | управляет | **не управляет** — это не сервис Compose |
| Профили и взаимное исключение слотов | действуют | не действуют, VRAM чужая |
| Расчёт VRAM из этого документа | применим | считается на том хосте |
| Метрики в Grafana | собираются Alloy | **не собираются** без правки `observability/config.alloy` |
| Health-проверка | healthcheck Compose | `health_check_interval` LiteLLM, раз в 30 секунд |

Три вещи, о которых легко забыть:

- **Сеть.** LiteLLM стоит в сети `backend`, исходящие соединения наружу разрешены, поэтому LAN-адрес доступен без дополнительной настройки. А вот встречный доступ нужно закрыть: порт 8000 удалённого vLLM не должен торчать в общую сеть, ограничьте его firewall до адреса этого хоста.
- **Трафик открытый.** LiteLLM ходит к удалённому vLLM по обычному HTTP, ключ передаётся заголовком. Внутри доверенного сегмента это приемлемо; через недоверенные сети нужен VPN или TLS-терминатор перед vLLM.
- **Ключ в `/proc`.** На удалённом хосте ключ передан переменной окружения, а не аргументом `--api-key`, ровно по той же причине, что и в локальном `compose.yaml`: аргументы командной строки читает любой локальный пользователь.

## Рецепт 5: пул серверов под одним именем модели

Если несколько записей имеют **одинаковый** `model_name`, LiteLLM считает их деплойментами одной модели и распределяет запросы между ними. Так пул расширяется горизонтально, а клиенты продолжают просить одно имя.
...
```yaml
  ...
  - model_name: qwen3.5-9b-awq          # то же имя — второй сервер того же пула
    litellm_params:
      model: openai/qwen3.5-9b-awq
      api_base: http://10.0.0.6:8000/v1
      api_key: os.environ/REMOTE_VLLM_KEY_2
      timeout: 600
```

Стратегия распределения задаётся в `router_settings`; в этом репозитории уже стоит `simple-shuffle` — случайный выбор. С одним деплойментом она ничего не делала, с пулом начинает работать. Доступны также `least-busy`, `latency-based-routing`, `usage-based-routing` и `cost-based-routing`.

Приоритет и запасной вариант задаются параметром `order`: меньшее значение — выше приоритет, при отказах LiteLLM переходит к следующему уровню. При ответе `429` деплоймент немедленно уходит в cooldown, и трафик идёт на оставшиеся.

**Ловушка, которая стоит дорого.** Все записи с одним `model_name` обязаны отдавать **одну и ту же модель той же revision**. LiteLLM это не проверяет: если на одном сервере 9B, а на другом 4B, клиенты будут получать ответы разного качества случайным образом, и воспроизвести жалобу не выйдет. Разные модели — разные `model_name`.
...
Когда VRAM этого хоста закончилась, пул расширяется вширь, а не вглубь: модель поднимается на другом сервере и подключается к LiteLLM записью в конфигурации — см. [Рецепт 4](#рецепт-4-vllm-на-отдельном-сервере). Несколько серверов можно объединить под одним именем модели с распределением запросов — [Рецепт 5](#рецепт-5-пул-серверов-под-одним-именем-модели).
